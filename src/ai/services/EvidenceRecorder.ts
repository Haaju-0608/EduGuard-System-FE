...
export class EvidenceRecorder {
  ...
  private async upload(
    ...
    const preEventBufferMs = this.options.maxChunks * this.options.chunkMs;
    const formData = new FormData();
    formData.append('video', videoBlob, `${violation.type}-${Date.now()}.webm`);
    formData.append('violationType', violation.type);
    formData.append('participationId', this.options.participationId);
    formData.append('sessionId', this.options.sessionId);
    formData.append('studentId', this.options.studentId);
    formData.append('timestamp', timestampIso);
    formData.append('direction', violation.metadata.gaze ?? '');
    formData.append('confidence', String(violation.metadata.signalConfidence ?? ''));
    formData.append('duration', String(Math.round(durationMs / 1000)));
    formData.append('metadata', JSON.stringify({
      evidenceDurationMs: durationMs,
      preEventBufferMs,
      postEventRecordingMs: this.options.postViolationMs,
      videoBitsPerSecond: this.options.videoBitsPerSecond,
      captureWidth: DEFAULT_CAPTURE_WIDTH,
      frameIntervalMs: this.options.chunkMs,
