...
/** Tự thêm mọi sinh viên đang active của lớp làm participant ngay khi tạo exam slot — theo yêu cầu
 *  team test (trước đó phải Import Excel/tự add tay mới có roster, School Admin thấy bất tiện vì
 *  hầu hết trường hợp chỉ cần "cả lớp thi chung"). Best-effort, chạy nền sau khi tạo slot thành
 *  công — 1 sinh viên lỗi (vd trùng giờ thi khác) không chặn các sinh viên còn lại; vẫn có thể
 *  thêm/xoá tay hoặc Import Excel bù qua modal Participants nếu cần chỉnh sau. */
async function autoImportParticipantsForNewSlot(examSlotId: string, classId: string) {
  try {
    ...
    await Promise.allSettled(
      activeStudentIds.map((studentId) => createExamParticipation({ examSlotId, studentId })),
    );
  } catch {
    // best-effort — không chặn cảm giác "tạo exam thành công", admin vẫn thêm tay được sau
  }
}
      ...
      if (isEdit) {
        ...
        if (succeededIds.length > 0) {
          ...
          results.forEach((result, i) => {
            if (result.status === 'fulfilled') void autoImportParticipantsForNewSlot(result.value.id, form.classIds[i]);
          });
        }
        if (failed.length > 0) {
